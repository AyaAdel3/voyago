...
// صفحة الـ Budget Planning (محتاجة auth):
...
//   - السلايدر هنا صفحة كاملة (pageSize) + نقط، بالظبط زي details.ts
// ============================================================
...
export class Plan implements OnInit {
  ...
  // الحد الأدنى التقريبي للميزانية اليومية (فندق + أكل + أنشطة) بالجنيه
  private readonly MIN_DAILY_BUDGET = 300;

  get minRequiredBudget(): number {
    return this.days > 0 ? this.days * this.MIN_DAILY_BUDGET : 0;
  }
  ...
  // ── Slider state — نفس نظام details (pageSize بيتغير حسب الشاشة) ──
  ...
  private validateBudget(): void {
    if (this.totalBudget && this.totalBudget > 0 && this.totalBudget < this.minRequiredBudget) {
      this.inputError = `⚠ Minimum budget for ${this.days} day${this.days > 1 ? 's' : ''} is ${this.minRequiredBudget.toLocaleString()}LE`;
    } else {
      this.inputError = '';
    }
  }
  ...
  generatePlan(): void {
    ...
    if (this.totalBudget < this.minRequiredBudget) {
      this.inputError = `⚠ Minimum budget for ${this.days} day${this.days > 1 ? 's' : ''} is ${this.minRequiredBudget.toLocaleString()}LE`;
      return;
    }
  ...
  addRestaurantToPlan(r: SuggestedRestaurantItem): void {
    ...
    const newTotal = this.restaurantBudgetUsed + r.estimatedPrice;
  ...
  get restaurantBudgetRemaining(): number {
    return this.breakdown.restaurantBudget - this.restaurantBudgetUsed;
  }
  ...
  get attractionBudgetRemaining(): number {
    return this.breakdown.attractionBudget - this.selectedAttractionsCost;
  }
